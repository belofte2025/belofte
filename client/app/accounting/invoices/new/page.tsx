...
import { ArrowLeft, FileText, Search, User, Trash2, Calendar } from "lucide-react";
...
export default function NewInvoicePage() {
  ...
  const addItem = (item: SupplierItem) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { itemName: item.itemName, supplierName: item.supplierName, quantity: 1, unitPrice: item.unitPrice },
      ];
    });
  };

  const updateCart = (idx: number, field: "quantity" | "unitPrice", value: string) => {
    const num = parseFloat(value);
    if (isNaN(num) || num < 0) return;
    setCart((prev) => prev.map((c, i) => (i === idx ? { ...c, [field]: field === "quantity" ? Math.max(1, num) : num } : c)));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const invoice = await createInvoice({
        ...
        items: cart.map((c) => ({ itemName: c.itemName, quantity: c.quantity, unitPrice: c.unitPrice })),
      });
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-3 mb-6">
            <Link href="/accounting/invoices" className="p-2 text-gray-400 hover:text-gray-600 hover:bg-white rounded-lg transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">New Invoice</h1>
              <p className="text-sm text-gray-500">Create a customer invoice</p>
            </div>
          </div>

          <div className="grid lg:grid-cols-3 gap-6">
            {/* Left sidebar — details + cart */}
                      ...
                      <div key={idx} className="bg-gray-50 rounded-lg p-3 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div>
                            ...
                            <input
                              type="number" min="0" step="0.01" className="input text-xs py-1"
                              value={entry.unitPrice} onChange={(e) => updateCart(idx, "unitPrice", e.target.value)}
                            />
                          </div>
                        </div>
