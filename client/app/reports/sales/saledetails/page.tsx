    ...
    try {
      ...
      const html = `
        <html>
          ...
          <body>
            <h2>${search ? search.toUpperCase() + ' ' : ''}Sales Summary Report</h2>
            <p><strong>Date Period:</strong> ${new Date(startDate).toLocaleDateString()} to ${new Date(endDate).toLocaleDateString()}</p>
            ${filtered
              .map(
                (s) => `
              <div class="sale">
                <h3>Customer: ${s.customerName} (${s.saleType})</h3>
                <p><strong>Date:</strong> ${new Date(s.createdAt).toLocaleDateString()}</p>
                <table>
                  <thead>
                    <tr>
                      <th>Item Name</th>
                ...
                <p class="total">Total: GHS ${formatCurrency(s.items.reduce((sum, i) => sum + i.total, 0))}</p>
              </div>
            `
              )
              .join("")}
      ...
      html2pdf().from(html).save(`Supplier_Sales_Summary_${startDate}_to_${endDate}.pdf`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Supplier Sales Summary</h1>
                <p className="mt-1 text-gray-600">Detailed supplier-wise sales breakdown</p>
              </div>
            </div>
            ...
            <>
              ...
              <div className="space-y-4 mb-6">
                {filtered.map((sale) => {
                  ...
                  return (
                    <div key={sale.saleId} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
